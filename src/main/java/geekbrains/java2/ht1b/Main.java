...
public class Main {
    ...
    private static void printBoard(){
        ...
        for (String[] aBoard : board) {
            ...
            for (int j = 0; j < aBoard.length; j++) {
                System.out.print(getValue(aBoard[j]) + " | ");
    ...
    private static boolean validMove(String move) {
        ...
        if(move.equalsIgnoreCase("C2") && (board[2][1] == "C2"))
            return true;
        return move.equalsIgnoreCase("C3") && (board[2][2] == "C3");
    }
    ...
    private static  String computerMove() {
        ...
        for(int i = 0; i < 3; i++){
            if (board[i][0] == board[i][1] && validMove(board[i][2])) {
                return board[i][2];
            } else if (board[i][0] == board[i][2] && validMove(board[i][1])) {
                return board[i][1];
            } else if (board[i][1] == board [i][2] && validMove(board[i][0])) {
                return board[i][0];
            }
        }
        ...
        for(int j = 0; j < 3; j++) {
            if (board[0][j] == board[1][j] && validMove(board[2][j])) {
                return board[2][j];
            } else if (board[0][j] == board[2][j] && validMove(board[1][j])) {
                return board[1][j];
            } else if (board[1][j] == board[2][j] && validMove(board[0][j])) {
                return board[0][j];
            }
        }

        //блокируем линию по диагонали
        if(board[0][0] == board[1][1] && validMove(board[2][2])) {
            return board[2][2];
        } else if (board[0][0] == board[2][2] && validMove(board[1][1])) {
            return board[1][1];
        } else if (board[1][1] == board[2][2] && validMove(board[0][0])) {
            return board[0][0];
        }

        if(board[0][2] == board[1][1] && validMove(board[2][0])) {
            return board[2][0];
        } else if (board[0][2] == board[2][0] && validMove(board[1][1])) {
            return board[1][1];
        } else if (board[1][1] == board[2][0] && validMove(board[0][2])) {
            return board[0][2];
        }
    ...
    private static boolean checkFullRow() {
        for (String[] aBoard : board) {
            if (checkRowValues(aBoard[0], aBoard[1], aBoard[2])) {
                return true;
            }
        }
        return false;
    ...
    private static boolean checkFullColumn() {
        for (int i = 0; i < board.length; i++) {
            if (checkRowValues(board[0][i], board[1][i], board[2][i])) {
                return true;
            }
        }
        return false;
    ...
    private static boolean checkFullDiagonal() {
        return checkRowValues(board[0][0], board[1][1], board[2][2]) || checkRowValues(board[0][2], board[1][1], board[2][0]);
    }

    // Проверяем есть ли победитель
    private static boolean winGame() {
        return checkFullRow() || checkFullColumn() || checkFullDiagonal();
    }
